...
> **Note:** Email Marketing is a **workspace product**, not a site-level module. You enable it at the workspace level and it works across every site in the workspace. Workspace contacts are also workspace-level — a single contact can interact with any of your sites.

---

## What Email Marketing Is

Think of Email Marketing as your own built-in newsletter and contact management tool. Once you turn it on, you get everything you need to collect email addresses, organize your contacts into lists, and stay in touch with your audience -- without needing a separate service like Mailchimp or Constant Contact.

Every contact you collect gets stored at the workspace level with their name, email, subscription status, and any custom information you want to track. You can organize contacts into lists (like "Newsletter" or "VIP Customers"), tag them for quick filtering (like "Attended Workshop" or "New Lead"), and manage their subscription preferences.
...
## How to Enable Email Marketing

Before you can start collecting contacts, you need to turn the product on.

1. Open your **account portal** and click the **Workspaces** card.
2. Open the workspace where you want to enable email marketing.
3. Click **Products** in the workspace settings area.
4. Find **Marketing** in the list of products.
5. Click **Enable**.

Once enabled, you will see a new **Marketing** section appear in your workspace sidebar. This section contains everything related to your contacts, lists, tags, templates, campaigns, automations, and marketing settings.

> **Tip:** When you enable Marketing, several related features are activated automatically, including mailing lists, tags, custom fields, and email templates. You do not need to enable these separately.

---

## Your Marketing Dashboard

After enabling the product, your workspace sidebar will show a **Marketing** section with the following items:
...
1. Click **Contact** under the Marketing section in your sidebar.
2. You will see a list of all your contacts with their name, email, status, and source.
3. Use the search bar to find a specific contact by name or email.
...
1. Click **Contact** in the Marketing section.
...
2. Update any of the fields -- name, phone, status, or lead score.
3. Save your changes.

### Deleting a Contact

1. Find the contact you want to remove in your contact list.
...
1. Click **Marketing List** in the Marketing section of your sidebar.
...
1. Click **Marketing Tag** in the Marketing section.
2. Click the **Create** button.
...
1. Click **Marketing Custom Field** in the Marketing section.
2. Click the **Create** button.
3. Fill in the field details:
   - **Field Key** (required) -- An internal identifier for the field (no spaces, use underscores). For example: `membership_level`.
   - **Label** (required) -- The display name for the field. For example: "Membership Level."
...
1. Click **Import** under the Marketing section in your sidebar.
2. Upload your CSV file (maximum 10 MB).
...
5. Optionally, select a mailing list to add all imported contacts to.
...
1. Their status changes from "Subscribed" to "Unsubscribed."
2. They are removed from all mailing lists.
3. The date and time of their unsubscription is recorded.
4. If they provided a reason, that is saved too.
...
Marketing campaigns can be sent from one of two places:

| Path | What recipients see | When to use |
|---|---|---|
| **WebNesting-hosted shared sender** (default) | `via webnesting.email` next to your name in some inboxes | Quick to start. Fine for small newsletters under ~5,000 sends/month or when you just want to try Marketing without DNS work. |
| **Your own verified domain** (recommended for any serious volume) | Clean — recipients see only your domain, no "via" tag, DKIM-signed by you | Required for high-volume sends, gated industries, or any campaign where deliverability matters. |

**Why use your own sending domain?**

- **Better deliverability.** Mail signed by your own domain bypasses many spam filters that treat shared senders cautiously.
- **Better branding.** Recipients don't see `via webnesting.email` in Gmail or other clients that show that "via" hint.
- **Your reputation, not ours.** Your campaign's bounce + complaint rates only affect your domain's reputation, not other WebNesting customers'.

### How to set it up

Marketing's sending domain uses the same domain-verification flow as Helpdesk's outbound — they share workspace-level verified domains and sender identities, so you only do this once even if you also use Helpdesk.
...
### Why Marketing doesn't support Gmail OAuth or IMAP

Helpdesk has five inbound options (WebNesting-hosted, Gmail OAuth, Outlook OAuth, Vanity subdomain, IMAP). Marketing only supports two — the WebNesting-hosted shared sender or your verified domain. Why the restriction?

- **Gmail's per-day quota is ~2,000 emails.** Outlook's is ~10,000/day. Any non-trivial marketing campaign hits these limits and stops mid-send.
- **Recipients see "via gmail.com" in their inbox** when you send marketing through Gmail OAuth — which makes the email look like spam.
- **Bounce + complaint tracking** that drives our reputation safeguards needs DKIM-signed mail through our shared sender or your verified domain. We can't reliably track those signals through Gmail's API.

If you need branded outbound from Gmail-only infrastructure for low-volume notifications, use Helpdesk's Gmail OAuth option — Marketing is specifically for campaign-scale sending.

### Brand logo in recipient inbox (BIMI)

Once you have a verified domain set up for marketing sending, you can also show **your company logo next to outbound email** in supporting inboxes (Yahoo, La Poste, and — with a Verified Mark Certificate — Gmail and Apple Mail). See the [BIMI section of the Helpdesk email guide](../dashboard/helpdesk.md#brand-logo-in-recipient-inbox-bimi) for the full setup. The same BIMI configuration applies to both your marketing and helpdesk mail.
